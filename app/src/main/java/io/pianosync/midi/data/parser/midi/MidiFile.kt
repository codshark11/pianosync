...
class MidiFile(
    ...
    private var tracks: ArrayList<MidiTrack>? = null
    ...
    private fun EventName(ev: Int): String {
        if (ev >= EventNoteOff && ev < EventNoteOff + 16) return "NoteOff"
        else if (ev >= EventNoteOn && ev < EventNoteOn + 16) return "NoteOn"
        else if (ev >= EventKeyPressure && ev < EventKeyPressure + 16) return "KeyPressure"
        else if (ev >= EventControlChange && ev < EventControlChange + 16) return "ControlChange"
        else if (ev >= EventProgramChange && ev < EventProgramChange + 16) return "ProgramChange"
        else if (ev >= EventChannelPressure && ev < EventChannelPressure + 16) return "ChannelPressure"
        else if (ev >= EventPitchBend && ev < EventPitchBend + 16) return "PitchBend"
        else if (ev == MetaEvent.toInt()) return "MetaEvent"
        else if (ev == SysexEvent1.toInt() || ev == SysexEvent2.toInt()) return "SysexEvent"
        else return "Unknown"
    }

    /** Return a String representation of a meta-event  */
    private fun MetaName(ev: Int): String {
        if (ev == MetaEventSequence.toInt()) return "MetaEventSequence"
        else if (ev == MetaEventText.toInt()) return "MetaEventText"
        else if (ev == MetaEventCopyright.toInt()) return "MetaEventCopyright"
        else if (ev == MetaEventSequenceName.toInt()) return "MetaEventSequenceName"
        else if (ev == MetaEventInstrument.toInt()) return "MetaEventInstrument"
        else if (ev == MetaEventLyric.toInt()) return "MetaEventLyric"
        else if (ev == MetaEventMarker.toInt()) return "MetaEventMarker"
        else if (ev == MetaEventEndOfTrack.toInt()) return "MetaEventEndOfTrack"
        else if (ev == MetaEventTempo.toInt()) return "MetaEventTempo"
        else if (ev == MetaEventSMPTEOffset.toInt()) return "MetaEventSMPTEOffset"
        else if (ev == MetaEventTimeSignature.toInt()) return "MetaEventTimeSignature"
        else if (ev == MetaEventKeySignature.toInt()) return "MetaEventKeySignature"
        else return "Unknown"
    }


    /** Get the list of tracks  */
    fun getTracks(): ArrayList<MidiTrack> {
        return tracks!!
    }
    ...
    private fun parse(rawdata: ByteArray?) {
        ...
        allevents = ArrayList<ArrayList<MidiEvent>>()
        for (tracknum in 0..<num_tracks) {
            allevents!!.add(ReadTrack(file))
            val track = MidiTrack(allevents!!.get(tracknum), tracknum)
        ...
        Companion.CheckStartTimes(tracks!!)
        ...
        timesig = TimeSignature(numer, denom, quarternote, tempo.toInt())
    }
            ...
            if (eventflag >= EventNoteOn && eventflag < EventNoteOn + 16) {
                ...
                if (mevent.Metaevent == MetaEventTimeSignature) {
                    if (mevent.Metalength < 2) {
                        ...
                        mevent.Denominator = (2.0.pow(mevent.Value!![1].toDouble()).toInt().toByte())
                    }
                ...
                throw MidiFileException(
                    "Unknown event " + mevent.EventFlag,
                    file.GetOffset() - 1
                )
            }
        }
    ...
    fun ApplyOptionsToEvents(options: MidiOptions): ArrayList<ArrayList<MidiEvent>> {
        ...
        var newevents: ArrayList<ArrayList<MidiEvent>> = Companion.CloneMidiEvents(allevents!!)

        /* Set the tempo at the beginning of each track */
        for (tracknum in newevents.indices) {
            val mevent: MidiEvent = CreateTempoEvent(options.tempo)
            newevents.get(tracknum).add(0, mevent)
        }

        /* Change the note number (transpose), instrument, and tempo */
        for (tracknum in newevents.indices) {
            for (mevent in newevents.get(tracknum)) {
                var num: Int = mevent.Notenumber + options.transpose
                if (num < 0) num = 0
                if (num > 127) num = 127
    ...
    fun ApplyOptionsPerChannel(options: MidiOptions): ArrayList<ArrayList<MidiEvent>> {
        ...
        for (tracknum in tracks!!.indices) {
            val track = tracks!!.get(tracknum)
            val channel = track.notes.get(0).channel
    ...
    fun ChangeMidiNotes(options: MidiOptions): ArrayList<MidiTrack> {
        ...
        Companion.RoundStartTimes(newtracks, options.combineInterval, timesig!!)
        RoundDurations(newtracks, time.quarter)

        if (options.twoStaffs) {
            newtracks = CombineToTwoTracks(newtracks, timesig!!.measure)
        }
    ...
    fun GuessMeasureLength(): ListInt {
        ...
        val pulses_per_second = (1000000.0 / timesig!!.tempo * timesig!!.quarter).toInt()
        ...
        var firstnote = timesig!!.measure * 5
        for (track in tracks!!) {
            if (firstnote > track.notes.get(0).startTime) {
                firstnote = track.notes.get(0).startTime
            }
        }

        /* interval = 0.06 seconds, converted into pulses */
        val interval = timesig!!.quarter * 60000 / timesig!!.tempo
    ...
    fun EndTime(): Int {
        ...
        for (track in tracks!!) {
            ...
            val last = track.notes.get(track.notes.size - 1).startTime
            lastStart = max(last, lastStart)
        }
        return lastStart
    }
    ...
    companion object {
        ...
        val EventNoteOff: Byte = 0x80.toByte()
        val EventNoteOn: Byte = 0x90.toByte()
        val EventKeyPressure: Byte = 0xA0.toByte()
        val EventControlChange: Byte = 0xB0.toByte()
        val EventProgramChange: Byte = 0xC0.toByte()
        val EventChannelPressure: Byte = 0xD0.toByte()
        val EventPitchBend: Byte = 0xE0.toByte()
        val SysexEvent1: Byte = 0xF0.toByte()
        val SysexEvent2: Byte = 0xF7.toByte()
        val MetaEvent: Byte = 0xFF.toByte()

        /* The list of Meta Events */
        val MetaEventSequence: Byte = 0x0.toByte()
        val MetaEventText: Byte = 0x1.toByte()
        val MetaEventCopyright: Byte = 0x2.toByte()
        val MetaEventSequenceName: Byte = 0x3.toByte()
        val MetaEventInstrument: Byte = 0x4.toByte()
        val MetaEventLyric: Byte = 0x5.toByte()
        val MetaEventMarker: Byte = 0x6.toByte()
        val MetaEventEndOfTrack: Byte = 0x2F.toByte()
        val MetaEventTempo: Byte = 0x51.toByte()
        val MetaEventSMPTEOffset: Byte = 0x54.toByte()
        val MetaEventTimeSignature: Byte = 0x58.toByte()
        val MetaEventKeySignature: Byte = 0x59.toByte()
     ...
     * name.
     */
        var Instruments: Array<String?> = arrayOf<String?>(
        ...
        private fun WriteEvents(
            file: FileOutputStream, allevents: ArrayList<ArrayList<MidiEvent>>,
            trackmode: Int, quarter: Int
        ) {
            val buf = ByteArray(16384)

            /* Write the MThd, len = 6, track mode, number tracks, quarter note */
            file.write("MThd".toByteArray(StandardCharsets.US_ASCII), 0, 4)
            IntToBytes(6, buf, 0)
            file.write(buf, 0, 4)
            ...
            for (list in allevents) {
                ...
                val len: Int = GetTrackLength(list)
                IntToBytes(len, buf, 0)
                file.write(buf, 0, 4)

                for (mevent in list) {
                    val varlen: Int = VarlenToBytes(mevent.DeltaTime, buf, 0)
                    ...
                    if (mevent.EventFlag == EventNoteOn) {
                        ...
                        val offset: Int = VarlenToBytes(mevent.Metalength, buf, 0)
                        ...
                        buf[0] = mevent.Metaevent
                        val offset: Int = VarlenToBytes(mevent.Metalength, buf, 1) + 1
                        ArrayCopy(mevent.Value!!, 0, buf, offset, mevent.Value!!.size)
                        file.write(buf, 0, offset + mevent.Value!!.size)
                    }
                }
        ...
        private fun CloneMidiEvents(origlist: ArrayList<ArrayList<MidiEvent>>): ArrayList<ArrayList<MidiEvent>> {
            val newlist =
                ArrayList<ArrayList<MidiEvent>>(origlist.size)
            for (tracknum in origlist.indices) {
                val origevents = origlist.get(tracknum)
         ...
         */
        private fun StartAtPauseTime(
            list: ArrayList<ArrayList<MidiEvent>>,
            pauseTime: Int
        ): ArrayList<ArrayList<MidiEvent>> {
            val newlist = ArrayList<ArrayList<MidiEvent>>(list.size)
            for (tracknum in list.indices) {
                val events = list.get(tracknum)
     ...
     */
        private fun FindHighLowNotes(
            notes: ArrayList<MidiNote>, measurelen: Int, startindex: Int,
            ...
            while (i < notes.size && notes.get(i).startTime < endtime) {
                if (notes.get(i).endTime < starttime) {
                    i++
                    continue
                }
                if (notes.get(i).startTime + measurelen < starttime) {
                    i++
                    continue
                }
                if (pair.high < notes.get(i).number) {
                    pair.high = notes.get(i).number
                }
                if (pair.low > notes.get(i).number) {
                    pair.low = notes.get(i).number
                }
                i++
            }
        }
        ...
        private fun FindExactHighLowNotes(
            notes: ArrayList<MidiNote>, startindex: Int, starttime: Int,
            pair: PairInt
        ) {
            var i = startindex

            while (notes.get(i).startTime < starttime) {
                i++
            }

            while (i < notes.size && notes.get(i).startTime == starttime) {
                if (pair.high < notes.get(i).number) {
                    pair.high = notes.get(i).number
                }
                if (pair.low > notes.get(i).number) {
     ...
     */
        fun SplitTrack(track: MidiTrack, measurelen: Int): ArrayList<MidiTrack> {
            ...
            for (note in notes) {
                ...
                while (notes.get(startindex).endTime < note.startTime) {
                    startindex++
                }
            ...
            Collections.sort<MidiNote>(top.notes)
            Collections.sort<MidiNote>(bottom.notes)
        ...
        fun CombineToSingleTrack(tracks: ArrayList<MidiTrack>): MidiTrack {
            ...
            for (tracknum in tracks.indices) {
                ...
                notecount[tracknum] = tracks.get(tracknum).notes.size
            }
            ...
            while (true) {
                ...
                for (tracknum in tracks.indices) {
                    ...
                    val note = track.notes.get(noteindex[tracknum])
                    if (lowestnote == null) {
                        lowestnote = note
         ...
         * track.
         */
        fun CombineToTwoTracks(
            tracks: ArrayList<MidiTrack>,
            measurelen: Int
        ): ArrayList<MidiTrack> {
            val single: MidiTrack = CombineToSingleTrack(tracks)
            val result: ArrayList<MidiTrack> = SplitTrack(single, measurelen)
            ...
            if (lyrics.size > 0) {
                Collections.sort<MidiEvent?>(lyrics, lyrics.get(0))
        ...
        fun RoundStartTimes(tracks: ArrayList<MidiTrack>, millisec: Int, time: TimeSignature) {
            ...
            for (track in tracks) {
                ...
                Collections.sort<MidiNote>(track.notes)
            }
        }
        ...
        fun RoundDurations(tracks: ArrayList<MidiTrack>, quarternote: Int) {
            for (track in tracks) {
                var prevNote: MidiNote? = null
                for (i in 0..<track.notes.size - 1) {
                    val note1 = track.notes.get(i)
                    if (prevNote == null) {
                        prevNote = note1
                    }
                    ...
                    var dur = 0
                    if (quarternote <= maxduration) dur = quarternote
                    else if (quarternote / 2 <= maxduration) dur = quarternote / 2
                    else if (quarternote / 3 <= maxduration) dur = quarternote / 3
                    else if (quarternote / 4 <= maxduration) dur = quarternote / 4
        ...
        private fun SplitChannels(
            ...
            for (note in origtrack.notes) {
                ...
                if (!foundchannel) {
                    ...
                    track.AddNote(note)
                    track.setInstrument(channelInstruments[note.channel])
                    result.add(track)
                }
            }
            ...
            if (lyrics != null) {
                for (lyricEvent in lyrics) {
                    if (lyricEvent != null) {
                        for (track in result) {
                            if (lyricEvent.Channel.toInt() == track.notes.get(0).channel) {
        ...
        fun hasMidiHeader(data: ByteArray?): Boolean {
            val s: String?
            s = kotlin.text.String(data!!, 0, 4, StandardCharsets.US_ASCII)
            return s == "MThd"
        }
