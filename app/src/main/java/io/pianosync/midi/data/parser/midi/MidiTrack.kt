...
class MidiTrack {
    ...
    constructor(events: ArrayList<MidiEvent>, tracknum: Int) {
        this.tracknum = tracknum
        notes = ArrayList<MidiNote>(events.size)
        instrument = 0

        for (mevent in events) {
            if (mevent.EventFlag == MidiFile.Companion.EventNoteOn && mevent.Velocity > 0) {
                val note =
                    MidiNote(mevent.StartTime, mevent.Channel.toInt(), mevent.Notenumber.toInt(), 0)
                AddNote(note)
            } else if (mevent.EventFlag == MidiFile.Companion.EventNoteOn && mevent.Velocity.toInt() == 0) {
                NoteOff(mevent.Channel.toInt(), mevent.Notenumber.toInt(), mevent.StartTime)
            } else if (mevent.EventFlag == MidiFile.Companion.EventNoteOff) {
                NoteOff(mevent.Channel.toInt(), mevent.Notenumber.toInt(), mevent.StartTime)
            } else if (mevent.EventFlag == MidiFile.Companion.EventProgramChange) {
                instrument = mevent.Instrument.toInt()
            } else if (mevent.Metaevent == MidiFile.Companion.MetaEventLyric) {
                AddLyric(mevent)
                if (lyrics == null) {
                    lyrics = ArrayList<MidiEvent?>()
                }
                lyrics!!.add(mevent)
            }
        }
    ...
    val instrumentName: String?
        get() {
            if (instrument >= 0 && instrument <= 128) return MidiFile.Companion.Instruments[instrument]
            else return ""
        }
    ...
    fun Clone(): MidiTrack {
        ...
        if (lyrics != null) {
            track.lyrics = ArrayList<MidiEvent?>()
