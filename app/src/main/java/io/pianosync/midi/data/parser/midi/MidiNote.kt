...
/** The duration, in pulses  */ /* Create a new MidiNote.  This is called when a NoteOn event is
     ...
     */
    override fun compareTo(other: MidiNote): Int {
        if (this.startTime == other.startTime) return this.number - other.number
    ...
    fun Clone(): MidiNote {
        return MidiNote(
            this.startTime, channel,
            this.number, duration
        )
    }

    override fun toString(): String {
        val scale: Array<String> =
            arrayOf("A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#")
        return String.format(
            "MidiNote channel=%1\$s number=%2\$s %3\$s start=%4\$s duration=%5\$s",
            channel, this.number, scale[(this.number + 3) % 12], this.startTime, duration
        )
    }
}
